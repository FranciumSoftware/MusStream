    ...
    )
{
    ...
    query.bindValue(":name", name);          // Replace `name` with track name
    query.bindValue(":artist", artist);      // Replace `artist` with artist name
    query.bindValue(":album", album);        // Replace `album` with album name
    query.bindValue(":year", year);          // Replace `year` with release year, integer
    query.bindValue(":url", url);            // Replace `url` with track URL
    query.bindValue(":url_cover", url_cover);// Replace `url_cover` with cover art URL
    query.bindValue(":lyrics", lyrics);      // Replace `lyrics` with lyrics text
    query.bindValue(":tags", tags);          // Replace `tags` with tags, e.g., genre
    query.bindValue(":fk_source", fk_source);// Replace `fk_source` with foreign key for source, integer
...
QList<QMap<QString, QVariant>> database::get10RandomMusics() {
    //qDebug() << "Beginning of function";
    QSqlQuery query;
    const QString request = "SELECT * FROM musics ORDER BY RANDOM() LIMIT 10" ;
    query.prepare(request);
    if (!query.exec()) {
        qWarning() << "Unable to get 10 random musics: " << query.lastError().text();
        return {};
    }
